...
func TestValidateUsersRejectsEmptyAndDuplicateTokens(t *testing.T) {
	if err := validateUsers([]option.CCMUser{{Name: "empty"}}); err == nil {
		t.Fatal("expected empty token to fail")
	}
	if err := validateUsers([]option.CCMUser{{Name: "a", Token: "token"}, {Name: "b", Token: "token"}}); err == nil {
...
func TestNewServiceRejectsUnauthenticatedNonLoopback(t *testing.T) {
	...
	_, err := NewService(t.Context(), log.NewNOPFactory().Logger(), "test", option.CCMServiceOptions{
		ListenOptions: option.ListenOptions{Listen: &addr},
	})
	if err == nil {
		t.Fatal("expected non-loopback ccm without users to fail")
	}
}
