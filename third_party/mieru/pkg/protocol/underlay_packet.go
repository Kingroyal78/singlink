...
type PacketUnderlay struct {
	...
	authTable           *serverAuthTable
	authCache           *serverAuthCache
...
func (u *PacketUnderlay) readOneSegment() (*segment, net.Addr, error) {
	for {
		...
		if u.isClient {
			...
			var decrypted bool
			var err error
			var matchedNewSessionUserName string
			...
			if !decrypted {
				...
				blockCipher, decryptedMeta, matchedNewSessionUserName, err = u.serverTryDecryptMetadataForNewSession(encryptedMeta, nonce, addr)
				if err == nil {
					decrypted = true
				}
			}
			if !decrypted {
				...
				if matchedNewSessionUserName != "" {
					u.authCache.record(addr, u.authTable, matchedNewSessionUserName)
				}
			}
		}
...
func (u *PacketUnderlay) serverTryDecryptMetadataForNewSession(encryptedMeta, nonce []byte, remoteAddr net.Addr) (cipher.BlockCipher, []byte, string, error) {
	var matchedBlock cipher.BlockCipher
	var decryptedMeta []byte
	var matchedUserName string
	table := u.authTable
	if table == nil {
		table = newServerAuthTable(u.users, 0)
	}
	triedUsers := make([]string, 0, serverAuthCacheMRUSize)

	for _, authUser := range u.authCache.candidates(remoteAddr, table) {
		if !cipher.CheckUserFromHint(authUser.nameBytes, nonce) {
			continue
		}
		triedUsers = append(triedUsers, authUser.name)
		block, decrypted, ok := serverTryDecryptAuthUser(encryptedMeta, nonce, authUser, true, false, true)
		if ok {
			matchedBlock = block
			decryptedMeta = decrypted
			matchedUserName = authUser.name
			break
		}
	}

	// Then, try to narrow down the user using the nonce hint.
	if matchedBlock == nil {
		for _, authUser := range table.users {
			if serverAuthUserWasTried(authUser.name, triedUsers) {
				continue
			}
			if !cipher.CheckUserFromHint(authUser.nameBytes, nonce) {
				continue
	...
	if matchedBlock == nil && !u.userHintIsMandatory {
		...
		for _, authUser := range table.users {
			...
			block, decrypted, ok := serverTryDecryptAuthUser(encryptedMeta, nonce, authUser, true, false, false)
			if ok {
				matchedBlock = block
				decryptedMeta = decrypted
				matchedUserName = authUser.name
	...
	return matchedBlock, decryptedMeta, matchedUserName, nil
}
