...
func (s *Session) input(seg *segment) error {
	...
	if seg.block != nil {
		...
		if !s.isClient {
			...
			metricType := s.userMetricType(userName)
			if s.uploadBytes == nil {
				s.uploadBytes = metrics.RegisterMetric(fmt.Sprintf(metrics.UserMetricGroupFormat, userName), metrics.UserMetricUploadBytes, metricType)
			}
			if s.downloadBytes == nil {
				s.downloadBytes = metrics.RegisterMetric(fmt.Sprintf(metrics.UserMetricGroupFormat, userName), metrics.UserMetricDownloadBytes, metricType)
			}
		}
	}
...
func (s *Session) checkQuota(userName string) (ok bool, err error) {
	...
	uploadCounter, ok := uploadBytes.(*metrics.Counter)
	if !ok || uploadCounter.Type() != metrics.COUNTER_TIME_SERIES {
		return true, fmt.Errorf("metric %s in group %s is not a time series counter", metrics.UserMetricUploadBytes, metricGroupName)
	}
	downloadCounter, ok := downloadBytes.(*metrics.Counter)
	if !ok || downloadCounter.Type() != metrics.COUNTER_TIME_SERIES {
		return true, fmt.Errorf("metric %s in group %s is not a time series counter", metrics.UserMetricDownloadBytes, metricGroupName)
	}
	for _, quota := range user.GetQuotas() {
		now := time.Now()
		then := now.Add(-time.Duration(quota.GetDays()) * 24 * time.Hour)
		totalBytes := uploadCounter.DeltaBetween(then, now)
		totalBytes += downloadCounter.DeltaBetween(then, now)
...
func (s *Session) userMetricType(userName string) metrics.MetricType {
	user, found := s.users[userName]
	if found && len(user.GetQuotas()) > 0 {
		return metrics.COUNTER_TIME_SERIES
	}
