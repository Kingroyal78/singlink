...
func NewMux(isClinet bool) *Mux {
	...
	if !isClinet {
		mux.authCache = newServerAuthCache()
	}
...
func (m *Mux) SetServerUsers(users map[string]*appctlpb.User) *Mux {
	...
	m.users = users
	m.authGeneration++
	m.authTable = newServerAuthTable(users, m.authGeneration)
	if m.authCache == nil {
		m.authCache = newServerAuthCache()
	}
	if m.used {
		...
		for _, underlay := range m.underlays {
			if udpUnderlay, ok := underlay.(*PacketUnderlay); ok {
				...
				udpUnderlay.authTable = m.authTable
				udpUnderlay.authCache = m.authCache
			}
		}
	}
	return m
...
func (m *Mux) acceptTCPUnderlay(rawListener net.Listener, properties UnderlayProperties) (Underlay, error) {
	...
	return m.serverWrapTCPConn(rawConn, properties.MTU(), m.users, m.authTable, m.authCache, trafficPattern), nil
}

func (m *Mux) serverWrapTCPConn(rawConn net.Conn, mtu int, users map[string]*appctlpb.User, authTable *serverAuthTable, authCache *serverAuthCache, trafficPattern *appctlpb.TrafficPattern) Underlay {
