...
func (t *StreamUnderlay) readOneSegment() (*segment, error) {
	...
	if t.recv == nil {
		decryptedMeta, matchedUserName, err = t.serverInitRecvBlockCipherAndDecryptMetadata(encryptedMeta)
		...
		if err != nil {
			...
			t.authCache.record(t.conn.RemoteAddr(), t.authTable, matchedUserName)
		}
	} else {
...
func (t *StreamUnderlay) serverInitRecvBlockCipherAndDecryptMetadata(encryptedMeta []byte) ([]byte, string, error) {
	if t.recv != nil {
	...
	table := t.authTable
	if table == nil {
		table = newServerAuthTable(t.users, 0)
	}
	triedUsers := make([]string, 0, serverAuthCacheMRUSize)

	for _, authUser := range t.authCache.candidates(t.conn.RemoteAddr(), table) {
		if !cipher.CheckUserFromHint(authUser.nameBytes, nonce) {
			continue
		}
		triedUsers = append(triedUsers, authUser.name)
		block, _, ok := serverTryDecryptAuthUser(encryptedMeta, nonce, authUser, true, false, true)
		if ok {
			matchedBlock = block
	...
	if matchedBlock == nil && !t.userHintIsMandatory {
		...
		for _, authUser := range table.users {
			...
			block, _, ok := serverTryDecryptAuthUser(encryptedMeta, nonce, authUser, true, false, false)
			if ok {
				matchedBlock = block
				matchedUserName = authUser.name
				break
	...
	return decryptedMeta, matchedUserName, nil
}
