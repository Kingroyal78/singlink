...
func TestCheckUserFromHintDoesNotAllocateForNormalUserName(t *testing.T) {
	user := []byte("user-1234")
	key := make([]byte, 32)
	...
	allocs := testing.AllocsPerRun(1000, func() {
		if !CheckUserFromHint(user, nonce) {
			t.Fatal("CheckUserFromHint() = false, want true")
		}
	})
	if allocs != 0 {
		t.Fatalf("CheckUserFromHint() allocs = %v, want 0", allocs)
	}
}
