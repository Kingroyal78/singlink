...
func CheckUserFromHint(user, nonce []byte) bool {
	...
	if len(user)+noncePrefixLenForUserHint <= 128 {
		var input [128]byte
		n := copy(input[:], user)
		...
		hash := sha256.New()
		_, _ = hash.Write(user)
		_, _ = hash.Write(nonce[:noncePrefixLenForUserHint])
		hash.Sum(output[:0])
	}
	return subtle.ConstantTimeCompare(output[:nonceSuffixLenForUserHint], nonce[len(nonce)-nonceSuffixLenForUserHint:]) == 1
}
