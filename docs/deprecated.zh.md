...
`rule_set_ip_cidr_match_source` 且已在 singlink 1.11.0 中被移除。
...
旧字段已在 singlink 1.12.0 中被移除。

#### 移除对 go1.18 和 go1.19 的支持

由于维护困难，singlink 1.10.0 要求至少 Go 1.20 才能编译。
...
GeoIP 已废弃且已在 singlink 1.12.0 中被移除。

maxmind GeoIP 国家数据库作为 IP 分类数据库，不完全适合流量绕过，
且现有的实现均存在内存使用大与管理困难的问题。

singlink 1.8.0 引入了[规则集](/zh/configuration/rule-set/)，
可以完全替代 GeoIP， 参阅 [迁移指南](/zh/migration/#迁移-geoip-到规则集)。

#### Geosite

Geosite 已废弃且已在 singlink 1.12.0 中被移除。
