...
!!! quote "Changes in singlink 1.13.3"
...
!!! failure "Deprecated in singlink 1.10.0"
    ...
    `inet4_address` is merged to `address` and will be removed in singlink 1.12.0.
...
!!! failure "Deprecated in singlink 1.10.0"
    ...
    `inet6_address` is merged to `address` and will be removed in singlink 1.12.0.
...
When unset, singlink derives one address per family by taking the next IP after
the first IPv4/IPv6 entry in [`address`](#address). Connections toward those
derived addresses are additionally hijacked into the singlink DNS module,
...
!!! failure "Deprecated in singlink 1.11.0"
    ...
    GSO has no advantages for transparent proxy scenarios, is deprecated and no longer works, and will be removed in singlink 1.12.0.
...
!!! question "Since singlink 1.12.18"
...
routing traffic to the singlink table only when no route is found in system tables.
...
When enabled, MPTCP connections will bypass singlink and connect directly, otherwise, will be rejected to avoid errors by default.
...
* When `auto_redirect` is enabled, `strict_route` also affects `SO_BINDTODEVICE` traffic:
    * Enabled: `SO_BINDTODEVICE` traffic is redirected through singlink.
    * Disabled: `SO_BINDTODEVICE` traffic bypasses singlink.
...
`inet4_route_address` is deprecated and will be removed in singlink 1.12.0, please use [route_address](#route_address)
instead.
...
`inet6_route_address` is deprecated and will be removed in singlink 1.12.0, please use [route_address](#route_address)
instead.
...
`inet4_route_exclude_address` is deprecated and will be removed in singlink 1.12.0, please
...
`inet6_route_exclude_address` is deprecated and will be removed in singlink 1.12.0, please
use [route_exclude_address](#route_exclude_address) instead.
